...
use Cline\Toggl\GroupRepositories\DatabaseGroupMembershipRepository;
...
/**
 * DatabaseGroupMembershipRepository test suite.
 *
 * Tests the database-backed group membership repository, which persists group
 ...
 */
describe('DatabaseGroupMembershipRepository', function (): void {
    /**
    ...
    beforeEach(function (): void {
        ...
        if (!Schema::hasTable('group_memberships')) {
            Schema::create('group_memberships', function ($table) use ($primaryKeyType, $morphType): void {
        ...
        $this->repository = new DatabaseGroupMembershipRepository(
            $this->registry,
        );
    ...
    describe('Happy Path', function (): void {
        ...
        test('uses custom table name', function (): void {
            ...
            config(['toggl.table_names.group_memberships' => 'custom_memberships']);

            $repository = new DatabaseGroupMembershipRepository(
                $this->registry,
            );
        ...
        test('uses custom database connection when configured', function (): void {
            ...
            // Assert
            expect($this->repository->isInGroup('beta-users', $context))->toBeTrue();
            expect(DB::table('group_memberships')->where('group_name', 'beta-users')->exists())->toBeTrue();
        });
        ...
        test('does not set connection when connection config is null', function (): void {
            ...
            expect(DB::table('group_memberships')->where('group_name', 'null-connection-test')->exists())->toBeTrue();
        });
    });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('getGroupMembers handles context_id as string', function (): void {
            ...
            DB::table('group_memberships')->insert(array_filter([
                'id' => $id,
