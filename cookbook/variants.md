...
```php
$variant = Feature::variant('checkout-flow');
// Returns: 'control', 'v1', or 'v2'

// Scoped to specific user
$variant = Feature::for($user)->variant('checkout-flow');
...
- Same scope always gets same variant
...
Feature::for($userA)->variant('experiment'); // 'v1'
Feature::for($userA)->variant('experiment'); // 'v1' (same)

// User B always gets 'control'
Feature::for($userB)->variant('experiment'); // 'control'
Feature::for($userB)->variant('experiment'); // 'control' (same)
```
...
```php
Feature::defineVariant('pricing-page', [
    'control' => 50,
    'price-first' => 25,
    'features-first' => 25,
]);

$layout = Feature::for($user)->variant('pricing-page');
...
$buttonColor = Feature::for($visitor)->variant('button-color');
```

### Gradual Feature Rollout

```php
Feature::defineVariant('new-editor', [
    'legacy' => 70,   // 70% stay on old
    'new' => 30,      // 30% get new editor
]);

if (Feature::for($user)->variant('new-editor') === 'new') {
...
```php
Feature::defineVariant('recommendation-algorithm', [
    'collaborative-filtering' => 33,
    'content-based' => 33,
    'hybrid' => 34,
]);

$algorithm = Feature::for($user)->variant('recommendation-algorithm');
...
```php
$variants = Feature::variantNames('checkout-flow');
// ['control', 'v1', 'v2']
```
...
```php
$config = Feature::getVariants('checkout-flow');
...
```php
if (Feature::getVariants('my-feature')) {
    // Feature has variants
    $variant = Feature::variant('my-feature');
} else {
    // Regular boolean feature
    $active = Feature::active('my-feature');
}
```
...
Feature::defineVariant('new-search', [
    'legacy' => 90,
    'new' => 10,
]);
...
Feature::define('new-search', true);
```
...
```blade
@php
    $variant = Feature::for(auth()->user())->variant('landing-page');
@endphp
...
Feature::defineVariant('price-test', [
    'control' => 50,
    'higher' => 25,
    'lower' => 25,
])
...
Feature::define('price-test', 'lower'); // Winner
```
...
$teamVariant = Feature::for($team)->variant('team-dashboard');

// Different experiments per user
$userVariant = Feature::for($user)->variant('onboarding-flow');
```
...
if (Feature::active('base-feature')) {
    $variant = Feature::variant('advanced-test');
}
```
...
1. **Keep experiments focused**
   ```php
   // ✅ Good - test one thing
   Feature::defineVariant('button-text', [
       'buy-now' => 50,
       'purchase' => 50,
   ]);
   
   // ❌ Avoid - too many variables
   Feature::defineVariant('everything', [
       'variant-a' => 10,
       'variant-b' => 10,
       // ... 8 more variants
   ]);
...
4. **Track everything**
   ```php
   $variant = Feature::variant('experiment');
...
5. **Plan your rollout**
   ```php
   // Phase 1: Small test
   Feature::defineVariant('feature', ['old' => 95, 'new' => 5]);
   
   // Phase 2: Increase if positive
   Feature::defineVariant('feature', ['old' => 50, 'new' => 50]);
   
   // Phase 3: Full rollout
   Feature::define('feature', 'new'); // Switch to boolean
   ```
...
test('variant returns consistent results', function () {
    Feature::defineVariant('test', ['a' => 50, 'b' => 50]);
    
    $user = User::factory()->create();
    
    $variant1 = Feature::for($user)->variant('test');
    $variant2 = Feature::for($user)->variant('test');
...
test('variant distribution is roughly correct', function () {
    ...
    for ($i = 0; $i < 1000; $i++) {
        $variant = Feature::for("user-{$i}")->variant('test');
        $results[$variant]++;
    }
